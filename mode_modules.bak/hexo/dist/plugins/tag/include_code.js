...
module.exports = ctx => function includeCodeTag(args) {
    ...
    const src = (0, path_1.join)(ctx.source_dir, codeDir, path);
    // If the title is not defined, use file name instead
    const title = match[1] || (0, path_1.basename)(path);
    const caption = `<span>${title}</span><a href="${path_1.posix.join(ctx.config.root, codeDir, path)}">view raw</a>`;
    return (0, hexo_fs_1.exists)(src).then(exist => {
        if (exist)
