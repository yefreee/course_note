...
2. 使用`swift list`命令插件当前用户下所创建的容器。
...
3. 创建一个文件并把它上传到容器`test_container`中。
...
4. 同样使用`swift list` + `容器名`可以列出指定容器内所包含的对象。
...
5. 使用`swift download`命令将对象从存储中下载到本地。
