...
swift post acl_container --read-acl "PROJECT_ID:*" --write-acl "PROJECT_ID:*"
```

提示：可以通过 `openstack project list` 或从管理员控制台获得 `PROJECT_ID`。

快速验证（示例流程）：

* 切换到非 admin 的用户（如 demo）：
...
* 以该用户访问公开容器：
    ...
    ```bash
    [root@controller ~]# curl -X GET http://controller:8080/v1/AUTH_TENANT_ID/acl_container
    ```

> 作业4：把 `acl_container` 设为公开并用非管理员账号（如 demo）用 `curl` 验证能否列出或下载对象，截屏上传。

（说明：上面命令中的 `AUTH_TENANT_ID` 或 `PROJECT_ID` 需按你的环境替换；也可先用 `swift auth` / `openstack token issue` 或 `openstack project list` 获取）
