...
<img src="https://vault.taojie.fun:28089/i/2023/11/18/2023-11-18-1700271777.png" alt="1700271777569.png" title="1700271777569.png" />

#### Swift对象的层次模型

<img src="https://vault.taojie.fun:28089/i/2023/11/16/2023-11-16-1700110449.png" alt="1700110449415.png" title="1700110449415.png" />
...
2. swift命令行工具的名称是`swift`，可通过`swift post`命令创建容器。

    ```shell
    [root@controller ~]# swift post test_container
    ```

3. 使用`swift list`命令插件当前用户下所创建的容器。
...
4. 创建一个文件并把它上传到容器`test_container`中。
...
5. 同样使用`swift list` + `容器名`可以列出指定容器内所包含的对象。
...
6. 使用`swift download`命令将对象从存储中下载到本地。
...
3. 登录Horizon控制台上传镜像并验证对象存储内的镜像文件。
    ...
    <img src="https://vault.taojie.fun:28089/i/2023/11/20/2023-11-20-1700465024.png" alt="1700465024334.png" title="1700465024334.png" />
...
1. 容器ACL示例1：
    ...
    退出当前的admin用户，使用demo用户登录查看容器`test_container`的信息。
...
2. 容器ACL示例2：
    ...
    此时用户demo无法查看到容器信息，因为demo不属于上述项目内。使用demo用户在身份管理菜单内添加demo到admin项目后能够再次看到容器信息。
