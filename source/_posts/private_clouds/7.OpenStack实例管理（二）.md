...
2. 打开新的计算节点后登录，这一步修改新添加的计算节点的网络配置，主机名。
    ...
    1. 修改主机名
        ...
        ```shell
        [root@compute ~]# hostnamectl set-hostname compute_2
        ```
    ...
    3. 修改网卡`ens33`和`ens34`的网络配置，将IP地址改为`192.168.10.21`和`192.168.20.21`
        ...
        ```shell
        [root@compute_2 ~]# vi /etc/sysconfig/network-scripts/ifcfg-ens33
        [root@compute_2 ~]# vi /etc/sysconfig/network-scripts/ifcfg-ens34
        [root@compute_2 ~]# ifdown ifcfg-ens33
        [root@compute_2 ~]# ifdown ifcfg-ens34
        [root@compute_2 ~]# ifup ifcfg-ens33
        [root@compute_2 ~]# ifup ifcfg-ens34
        ```
...
4. 重启`计算服务`。
    ...
    ```shell
    [root@compute_2 ~]# systemctl restart libvirtd openstack-nova-compute
    ```
...
6. 查看当前系统的计算节点信息。
    ...
    |  2 | compute_2           | QEMU            | 192.168.10.21 | up    |
    +----+---------------------+-----------------+---------------+-------+
    ```
...
1. 这一步设置两个计算节点之间的ssh免密访问，现在`compute`上配置。

    1. 查找`StrictHostKeyChecking`修改为`no`。

        ```shell
        [root@compute ~]# vi /etc/ssh/ssh_config
        ```

    2. 设置`nova`的`login`登录。
    ...
    3. 将前两步的配置发送到另一个计算节点。
        ...
        ```shell
        [root@compute ~]# scp /etc/ssh/ssh_config root@compute_2:/etc/ssh/ssh_config
        ```

    4. 设置nova密码，这一步在两个计算节点上设置。

        ```shell
        [root@compute ~]# passwd nova
        [root@compute_2 ~]# passwd nova
        ```
    ...
    6. 查看生成的公钥文件，复制粘贴到另一台机器的`/var/lib/nova/.ssh/authorized_keys`文件中。

        ```shell
        bash-4.2$ cat /var/lib/nova/.ssh/id_rsa.pub
        ```

    7. 另一台机器重复5、6两步，到目前为止两台计算节点能通过nova用户免密登录。
