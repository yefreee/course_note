...
1. 这一步设置两个计算节点之间的ssh免密访问，现在`compute`上配置。
    ...
    3. 将前两步的配置发送到另一个计算节点。
        ...
        ```shell
        [root@compute ~]# scp /etc/ssh/ssh_config root@compute2:/etc/ssh/ssh_config
        ```

    4. 用第三步scp命令发送修改好的`/etc/passwd`文件到compute2上。

    5. 设置nova密码，这一步在两个计算节点上设置。
    ...
    9. 在compute节点上验证ssh免密登录compute2。
