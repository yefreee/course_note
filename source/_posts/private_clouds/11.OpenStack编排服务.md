...
1. 新建模板文件`demo.yaml`并将以下内容保存到该模板文件：
...
4. 编写以下模板文件保存到`demo.yaml`文件中`resources`下，和`my_network`以及`my_subnet`平级：
...
6. 查看Horizon上的实例运行状态：
    ...
    > 作业2：在原有模板内添加实例资源后创建堆栈，查看堆栈资源列表后截图
...
在模板中可以通过`outputs`段返回堆栈创建完成后的相关信息，例如返回实例的IP地址、卷的UUID等。outputs段定义的内容可以通过OpenStack命令或API来获取，便于后续操作和集成。

1. 参考以下代码修改`demo.yaml`文件，添加outputs段：
    ...
    在outputs段中使用了`get_attr`内建函数来获取资源的属性值。
...
2. 通过传递参数创建堆栈：
    ...
    +---------------------+-----------------------------------------------------+
      | Field               | Value                                               |
    +---------------------+-----------------------------------------------------+
    | id                  | 5c2a6f1e-3d4b-4f8c-9a7e-1b2c3d4e5f6a                |
    | stack_name          | demo-struct                                         |
    | description         | Simple template to deploy a single compute instance |
    | creation_time       | 2023-12-04T10:15:22                                 |
    | updated_time        | None                                                |
    | stack_status        | CREATE_IN_PROGRESS                                  |
    | stack_status_reason | Stack CREATE started                                |
    +---------------------+-----------------------------------------------------+
...
3. 查看堆栈的输出结果：
    ...
    ```bash
    # 查看单个输出（instance_name）
    [root@controller ~]# openstack stack output show demo-struct instance_name
    +--------------+--------------------------------------+
    | Field        | Value                                |
    +--------------+--------------------------------------+
    | description  | Name of the instance                 |
    | output_key   | instance_name                        |
    | output_value | demo-struct-my_instance-7h8j9k0l1m2n |
    +--------------+--------------------------------------+
    ```

    ```bash
    # 查看全部输出
    [root@controller ~]# openstack stack output list demo-struct
    +---------------+--------------------------------------+
    ...
    >作业4：编写一个模板文件实现同时创建一个网络、子网和实例，要求实例连接到新创建的网络，并在outputs段输出网络ID和实例IP地址，创建堆栈后查看输出结果并截图上传。
    >
    >提示：使用`OS::Neutron::Net`创建网络，使用`OS::Neutron::Subnet`创建子网，注意资源之间的依赖关系。子网的网络地址可以使用`10.0.0.0/24`，网关设置为`10.0.0.1`。
