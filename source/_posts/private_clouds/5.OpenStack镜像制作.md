...
2. Windows + E打开文件资源管理器在搜索栏输入`\\192.168.3.47`打开后将文件`CentOS-7-x86_64-GenericCloud-2003.qcow2`拉取到Windows本地目录`D:\\镜像\`覆盖目录下的同名文件。
...
5. 创建Centos实例并使用root账户登录（密码是上文配置的123123）验证结果，应该会显示下图所示的屏保动画。
    ...
    <img src="https://vault.taojie.fun:28089/i/2023/10/13/2023-10-13-1697171723.png" alt="1697171722897.png" title="1697171722897.png" />
...
1. 在安装系统之前需要删除部分`C盘`内容后，获取系统安装包、VirtIO驱动，参考第一节从共享文件中拉取文件到桌面上`cn_windows_server_2012_r2_with_update_x64_dvd_6052725.iso`、`virtio-win-0.1.185.iso`、`WinSCP.exe`。
...
4. 选择`加载驱动程序`->`浏览`从以下路径加载scsi和网络驱动。
    ...
    > E:\virtio-win-0.1.185\viostor\2k12r2\amd64
    > E:\NETKVM\2k12r2\amd64
