...
1. 使用`ip addr`命令查看新的IP地址，ping下百度的域名检查外网连接状态，后续可以通过ssh远程连接。
...
3. 设置虚拟机和宿主机共享
    ...
    * 安装vm-tools并挂载设备
        ...
        [root@localhost ~]# vmhgfs-fuse .host:/ /mnt/hgfs/镜像-文档 -o subtype=vmhgfs-fuse,allow_other
...
4. 修改`/etc/libvirt/qemu.conf`，设置libvirt `root`权限和`VNC`远程虚拟机配置。
...
1. 执行virt-install命令启动Win-server操作系统。
    ...
    --disk path=/mnt/hgfs/镜像-文档/ws2012.qcow2,format=qcow2,device=disk,bus=virtio \
...
9. `localhost`虚拟机查看`ws2012`的运行状态。
