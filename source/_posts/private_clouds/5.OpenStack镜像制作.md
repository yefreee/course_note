...
1. 使用VMware安装fedora系统，内存6G，密码设置时可以统一使用`fedora00`。

2. 进入桌面后打开fedora的远程连接。
...
2. 从samba共享的文件中拉取`WinSCP-5.19.3`，使用第一步中获取到的ip地址和fedora的账户密码登录会话。
    ...
    <img src="https://vault.taojie.fun:28089/i/2023/10/09/2023-10-09-1696815764.png" alt="1696815764123.png" title="1696815764123.png" />

3. 将本地D盘的`CentOS-7-x86_64-GenericCloud-2003.qcow2`文件拉取到fedora系统中。

### 镜像定制

1. 在fedora桌面上打开终端执行`openssl passwd -l 123123`生成加密的字符串并`复制`用于设置root密码。
...
5. 在图形终端中执行`virt-manager`打开虚拟机控制同。通过控制台登录启动的windows server虚拟机完成安装。
...
6. 选择`加载驱动程序`->`浏览`从以下路径加载scsi和网络驱动。
...
8. 系统安装完成后在虚拟机管理器中点击查看 -> 详情 -> 引导选项，选中VirtIO磁盘。
...
9. 重启虚拟机，在以下启动界面按ESC后选择第二个分区启动系统。
...
15. 将生成的`ws2012.qcow2`传到`控制节点`上、上传到OpenStack后启动实例。
