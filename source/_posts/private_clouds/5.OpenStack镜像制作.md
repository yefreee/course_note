...
1. 在安装系统之前需要获取系统安装包、VirtIO驱动，从共享文件中拉取文件`cn_windows_server_2012_r2_with_update_x64_dvd_6052725.iso`、`virtio-win-0.1.185.iso`、`WinSCP.exe`。
...
8. 按`win`键输入`wf`选择进入高级安全Windows防火墙
...
1. 将生成的`ws2012.qcow2`传到Windows本地、然后关闭`xnode1`。

2. 打开控制节点和计算节点，上传`ws2012.qcow2`镜像到OpenStack后启动实例
