...
2. STDOUT(标准输出)
    ...
    在终端界面上，标准输出就是终端显示器。`shell`的所有正确信息（包括`shell`中运行的程序和脚本）会被定向到标准输出中，也就是显示器。

    默认情况下，大多数bash命令会将输出导向文件描述符`1`。
...
3. STDERR(标准错误)
    ...
    shell通过特殊的文件描述符`2`来处理错误消息。`shell`的所有错误信息（包括`shell`中运行的程序和脚本）会被定向到标准错误中，同样也是显示器。也就是说，默认情况下，错误消息也会输出到显示器输出中。
