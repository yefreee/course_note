...
root   tty1         2023-03-26 19:01
root   pts/0        2023-03-26 19:01 (192.168.13.1)
...
root@localhost:~$ ls -l test6
-rw-rw-r-- 1 root root 29 Mar 26 19:23 test6
root@localhost:~$ cat test6
Sun Mar 26 19:23:54 PDT 2023
root@localhost:~$
```

>作业1:理解并执行上述输入重定向、输出重定向的使用案例，完成后执行history命令截图。
...
- 只重定向错误信息
    ...
    ```shell
    root@localhost:~$ ls -al badfile 2> test3
    root@localhost:~$ cat test3
    ls: 无法访问 'badfile': 没有那个文件或目录 (No such file or directory)
    root@localhost:~$
    ```
...
- 重定向错误和正常输出
    ...
    ```shell
    root@localhost:~$ ls -al test3 test6 badtest 2>test7 1>test8
    root@localhost:~$ cat test7
    ls: 无法访问 'badtest': 没有那个文件或目录 (No such file or directory)
    root@localhost:~$ cat test8
    -rw-rw-r-- 1 root root  55 Mar 26 20:24 test3
    -rw-rw-r-- 1 root root 122 Mar 26 19:27 test6
    root@localhost:~$
    ```
    ...
    ```shell
    root@localhost:~$ ls -al badtest test3 test6 badtest &> test7
    root@localhost:~$ cat test7
    ls: 无法访问 'badtest': 没有那个文件或目录 (No such file or directory)
    ls: 无法访问 'badtest': 没有那个文件或目录 (No such file or directory)
    -rw-rw-r-- 1 root root  55 Mar 26 20:24 test3
...
root@localhost:~$ bash test.sh 2> test9
这是正常输出
...
>作业2：完成上述脚本内临时重定向和永久重定向两部分的练习，并执行history命令截图。
...
>作业3：编写上述脚本，实现在脚本内创建输入文件描述符并实现输入重定向。
...
```bash
root@localhost:~$ ls -al > /dev/null
root@localhost:~$
root@localhost:~$ ls -al badfile 2> /dev/null
root@localhost:~$
```
