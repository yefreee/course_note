...
>作业1：编写以下代码，通过函数实现模拟掷骰子的功能，运行脚本、截图。
...
- 单行方式定义函数
    ...
    ```bash
    root@localhost:~$ addem() { echo $(($1 + $2)); }
    root@localhost:~$ addem 1 2
    3
    ```
...
- 多行方式定义函数
    ...
    ```bash
    root@localhost:~$ multem() {
    > echo $(($1 * $2))
    > }
    root@localhost:~$ multem 2 4
    8
    root@localhost:~$
    ```
