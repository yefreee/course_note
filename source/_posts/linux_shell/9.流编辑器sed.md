...
>作业2：练习上述三节：常见选项、常见命令、其他命令，执行history截图。
...
4. 提取文本文件中的URL：
    ...
    样本数据 `url.txt`：
    ```text
    Welcome to our site: https://www.google.com
    Check this out: http://github.com/trending
    No URL here.
    ```

    ```bash
    sed -nE 's#.*(https?://[^ ]*).*#\1#p' url.txt
    ```

> 作业3：完成sed正则表达式用法的练习，执行history后截图
