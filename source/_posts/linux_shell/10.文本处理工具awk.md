...
* 用法示例：
    ...
    ```bash
    ❯  awk 'BEGIN { sum = 0 } { sum += $2 } END { print "Total Age:", sum }' test
    Total Age: 105
    ```
...
```bash
❯ awk '{printf "[Name:%-10s]\tAge:%s", $1, $2}' test
...
显示系统下各用户所使用的shell，查看当前`/etc/passwd`格式：
