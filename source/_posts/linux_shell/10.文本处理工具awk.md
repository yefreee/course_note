...
- 用法示例：
    ...
    ```bash
    # [计算] 累加第二列数值并输出总和
    ❯  awk 'BEGIN { sum = 0 } { sum += $2 } END { print "Total Age:", sum }' text.txt
    Total Age: 105
    ```
...
>作业1：以下是服务器产生的日志文件，将其保存到文件output.txt中，使用awk命令使其仅输出时间戳和CPU占用两列信息，上传awk命令的执行结果。
>扩展：在作业1的基础上使用grep正则表达式，进一步筛选并输出CPU占用超过80%的记录。
...
```bash
# [输出] 无逗号：字段直接拼接
❯ awk '{print "Name:" $1 "Age:" $2}' text.txt
Name:AliceAge:25
Name:BobAge:30
Name:CharlieAge:22
Name:DavidAge:35
...
```bash
# [printf] 行尾不会自动换行
❯ awk '{printf "[Name:%-10s]\tAge:%s", $1, $2}' text.txt
...
```bash
# [格式化] 输出姓名及 5 年后的年龄 (保留一位小数)
❯ awk '{printf "Name: %-8s | Future Age: %5.1f\n", $1, $2+5}' text.txt
Name: Alice    | Future Age:  30.0
Name: Bob      | Future Age:  35.0
Name: Charlie  | Future Age:  27.0
Name: David    | Future Age:  40.0
Name: Eve      | Future Age:  33.0
```
...
# [正确示例] 使用 -F 修改分隔符为逗号
❯ awk -F, '{print "Name:" $1, "Age:" $2}' text.txt
Name:Name Age:Age
Name:Alice Age:25
Name:Bob Age:30
Name:Charlie Age:22
...
>作业2：完成`记录与字段`部分的练习，执行history后截图。
...
David 35 Male Houston
Eve 28 Female Miami
# [过滤] 筛选第 2 列 (年龄) 小于 25 的记录
❯ awk '$2 < 25' text.txt
Charlie 22 Male Chicago
```

>扩展：使用awk命令按照以下格式输出年龄最大的人员信息：
...
格式化输出`text.txt`文件中所有以大写字母`A-C`开头的行：
...
David 35 Male Houston
Eve 28 Female Miami
❯ awk '/^[A-C]+.*/{print "Name:" $1}' text.txt
Name:Alice
Name:Bob
Name:Charlie
```

>作业3：使用awk，通过处理`/etc/passwd`来显示各用户使用的shell，按照以下格式输出，（如果用户没有使用shell则不显示）。
>提示：grep -v可用于显示不匹配的行。
