...
>作业1：先在`/home/`目录下创建名为`ubuntu`的目录，然后参考修改上一个脚本添加elif分支检测当用户不存在，但是存在同名目录时在目录中创建一个空文件，提交执行结果。
...
>作业2：参考以上文件比较、字符串比较、复合条件测试的内容，检查当前系统`/home`目录下是否存在名为`centos`的文件(文件比较)以及相应的目录(字符串比较)，如果都成立则在该目录下创建一个空文件，运行脚本查看结果。
