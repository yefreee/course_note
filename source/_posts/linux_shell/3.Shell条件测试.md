...
# 检查当前目录下是否存在 test.txt 文件
if ls test.txt
then
        echo "[提示] 文件已找到，准备开始处理。"
fi
```
...
```shell
[root@host1 ~]# rm -f test.txt
[root@host1 ~]# bash test.sh
ls: 无法访问 'test.txt': 没有那个文件或目录
[错误] 未找到文件 test.txt，请检查路径。
```
...
> 作业1：编写一个脚本，判断当前目录下是否存在 `movies` 目录。如果存在，则输出“准备好爆米花，电影时间到！”；如果不存在，则创建该目录并输出“目录已建好，快去下载电影吧！”。
...
- bash shell提供了另一种条件测试方法，无需在if-then语句中声明test命令。
  ...
  # 当变量 testuser 不为空时执行 then 语句的内容
  if [ $testuser ]
  then
          echo "变量 testuser 的值当前为: $testuser"
  fi
  ```
...
2. 字符串比较
    ...
    # 在条件比较时直接使用大于号 > 不能表示大于的含义，需要使用 \> 来表示“大于”
    if [ $var1 \> $var2 ]
    then
            echo "[结果] 字符串 $var1 大于 $var2"
    else
            echo "[结果] 字符串 $var1 不大于 $var2"
    fi
    ```
...
>作业2：编写脚本，检查当前用户 ID 是否为 0（即 root 用户），且 `/tmp` 目录是否存在。如果两个条件都满足，则在 `/tmp` 目录下创建一个名为 `test_success` 的文件。要求使用数值比较、文件比较和复合条件测试。
...
>3. 然后使用 **`case`** 语句输出对应数字`0-2`的英文，对于其他数字输出一个提示“该数字在3-9之间”。
