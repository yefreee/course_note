...
- 在命令行中使用分号组织命令  
    ...
    ```Shell 使用分号一次执行两个命令
    [root@host1 /]# date;date
    Tue Jun 13 14:16:40 UTC 2023
    Tue Jun 13 14:16:40 UTC 2023
    [root@host1 /]# date;whoami
...
- 第二种方式：将Shell脚本移动到`PATH`变量中包含的目录下。
...
>作业1: 编辑以下脚本，使用上述三种方式运行`test.sh`脚本
...
>作业2:修改上述脚本，将创建的文件名格式改为`主机名.当前日期`，如`localhost.230616`，执行脚本并查看创建的文件、截图。

- 从键盘读入赋值

    在Shell中，Shell变量可以通过从键盘读入输入的内容并来赋值。命令格式为：`read –p [提示信息]：[变量名]`
...
4. 变量的自增
    ...
    # 先取值再自增：var2拿到旧值
    var2=$((var1++))
    echo "执行 var2=$((var1++)) 后: var2=$var2, var1=$var1"
    ...
    执行 ((var1++)) 后: var1=11
    执行 ((++var1)) 后: var1=12
    执行 var2=$((var1++)) 后: var2=12, var1=13
    执行 var2=$((++var1)) 后: var2=14, var1=14
    ```
...
>作业3：编写脚本使用预定义参数来获取传递给脚本的所有参数和参数个数并打印，输出格式示例：参数个数为3，参数分别为123、abc、def。
